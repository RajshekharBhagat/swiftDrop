...
const PartnerList = () => {
  ...
  if (isLoading) {
    return <div className="flex flex-col md:min-h-screen min-h-[300px] items-center justify-center">
  ...
  if (!partners || partners.length === 0) {
    return <div className="flex flex-col md:min-h-screen min-h-[300px] items-center justify-center">
      <div className="flex flex-col items-center">
        ...
        <span className="text-lg text-gray-500 font-semibold">No Deliver Partners found.</span>
      </div>
    </div>;
  }
  return (
    <div className="max-w-6xl mx-auto w-full space-y-4 mt-10 px-4">
      {partners.map((partner) => (
        <div key={partner._id} className=" p-4 bg-violet-300 rounded-lg shadow">
          <div className="grid grid-cols-1 md:grid-cols-3 space-y-3">
            <div>
              <p className="md:text-lg text-sm font-bold">{partner.name}</p>
              <p className="md:text-sm text-xs text-zinc-700">{partner.email}</p>
              <p className="md:text-sm text-xs text-zinc-700">{partner.phone}</p>
            </div>
            <div className="flex flex-col space-y-3 justify-center">
              ...
              <span className="flex items-center gap-2 md:text-md text-xs font-semibold">Current Load: {partner.currentLoad}</span>
            </div>
            ...
            <span className="flex items-center gap-2 text-xs md:text-lg font-semibold">
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="max-w-md p-5 max-h-[90vh] overflow-y-auto">
                    <PartnerForm formHeading="Partner Profile" onSuccess={() => setIsDialogOpen(false)} selectedPartner={partner} buttonText="Update" />
                </DialogContent>
              </Dialog>
            </div>
          </div>
